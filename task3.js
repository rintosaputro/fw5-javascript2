const seleksiNilai = (nilaiAwal, nilaiAkhir, dataArray) => { // inisialisasi fungsi seleksiNilai dengan parameter nilaiAwal (number), nilaiAkhir(number), dataArray(array)
    if(typeof(nilaiAwal) == 'number' && typeof(nilaiAkhir) == 'number' && Array.isArray(dataArray)) { // validasi apakah nilai parameter nilaiAwal dan nilaiAkhir adalah number dan dataArray adalah array
    ...
    } else { // pengkondisian jika false dari validasi type data parameter seleksiNilai
        console.log('Inputan data tidak sesuai') // mencetak pesan untuk pengkondisian false
    }
}
